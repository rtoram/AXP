...
function renderFiles() {
    ...
    filtered.forEach(file => {
        ...
        li.innerHTML = `
            <span><i class="${iconClass} file-icon"></i> ${file.name} (${(file.size / 1024).toFixed(2)} KB) - ${file.date}</span>
            <div class="buttons">
...
function getFileIcon(type) {
    switch (type.toLowerCase()) {
        case 'jpg': case 'jpeg': case 'png': case 'gif': return 'fas fa-image';
...
function handleModalConfirm() {
    ...
    if (currentFileToRename) {
        ...
        saveFiles();
        closeModal();
        navigateToFolder(newFolder); // Abre a nova pasta imediatamente
    }
}
...
function updatePathDisplay() {
    const currentFolder = currentPath.join('/');
    const folderFiles = filesArray.filter(f => f.path === currentFolder);
    const totalSize = folderFiles.reduce((sum, file) => sum + file.size, 0);
    const sizeText = totalSize > 0 ? ` (${(totalSize / 1024).toFixed(2)} KB)` : '';
